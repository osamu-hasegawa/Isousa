...
namespace CameraDriver
{
	...
	{
		WM_GRAPHNOTIFY = 0x8001,	// 本バージョンでは使用しません
		WM_GRAPHPAINT,				// 映像が更新された時にアプリケーションに送られます
		WM_ERROR,					// エラーが発生した時にアプリケーションに送られます
	}
	...
	{
		public int		lSize;	// 構造体のサイズ sizeof(GP_INFO)
		public int		lWidth;	// 画像の幅;
		public int		lHeigh;	// 画像の高さt;
		public int		lBpp; 	// 画像のビット震度
		public int		lFps; 	// フレームレート（実際の10倍の値）
//		public byte[]	pImage;	// CallBackPreview等で指定した画像のポインタ
	}
	...
	public enum ARTCAM_FILTERTYPE
	{
		...
		ARTCAM_FILTERTYPE_GRAY_GAIN_AUTO,		// グレースケール化　オート

		ARTCAM_FILTERTYPE_CHANNEL_GAIN1,		// 左上チャンネルのグローバルゲイン
		ARTCAM_FILTERTYPE_CHANNEL_GAIN2,		// 右上チャンネルのグローバルゲイン
		ARTCAM_FILTERTYPE_CHANNEL_GAIN3,		// 左下チャンネルのグローバルゲイン
		ARTCAM_FILTERTYPE_CHANNEL_GAIN4,		// 右下チャンネルのグローバルゲイン
		ARTCAM_FILTERTYPE_CHANNEL_OFFSET1,		// 左上チャンネルのオフセット
		ARTCAM_FILTERTYPE_CHANNEL_OFFSET2,		// 右上チャンネルのオフセット
		ARTCAM_FILTERTYPE_CHANNEL_OFFSET3,		// 左下チャンネルのオフセット
		ARTCAM_FILTERTYPE_CHANNEL_OFFSET4,		// 右下チャンネルのオフセット

		// TNIRカメラ用
		ARTCAM_FILTERTYPE_PELTIER = 100,		// ペルチエ制御
		ARTCAM_FILTERTYPE_TEMPERATURE,			// センサー温度(取得のみ)
		ARTCAM_FILTERTYPE_DOTFILTER,			// ドットフィルター補正
		ARTCAM_FILTERTYPE_MASKFILTER,			// マスクフィルター補正
	}
	...
	{
		public int	lSize;				// 構造体のサイズ
		public int	lWidth;				// カメラの有効最大幅
		public int	lHeight;			// カメラの有効最大高さ
		public int	lGlobalGainMin;		// グローバルゲインの最低値（使用不可能のカメラは－１）
		public int	lGlobalGainMax;		// グローバルゲインの最低値（使用不可能のカメラは－１）
		public int	lColorGainMin;		// カラーゲインの最低値（使用不可能のカメラは－１）
		public int	lColorGainMax;		// カラーゲインの最低値（使用不可能のカメラは－１）
		public int	lExposureMin;		// 露光時間の最低値（使用不可能のカメラは－１）
		public int	lExposureMax;		// 露光時間の最低値（使用不可能のカメラは－１）
	}
